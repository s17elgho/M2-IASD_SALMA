...
def prepare_same_pairs(all_data,n_pairs_same,train_or_test):
  pairs_same = np.zeros((n_pairs_same,784,2))
...
def prepare_different_pairs(all_data, n_pairs_diff,train_or_test):
  ...
  pairs_diff = np.zeros((n_pairs_diff,784,2))
...
def threshold_plot(pred_test, y_test,pred_train, y_train):
  threshold_list = [i*0.015 for i in range (1,100)]
  ...
  for i in range(k): ## k is the number of times we repeat the process
    ...
    if typ == 'contrastive' or typ =='binary_crossentropy': 
      for classe in range(10):
        ...
        encodings_of_known_data[classe,:] = np.array(base_net.predict(example.reshape((1,28,28,1,1))))
      ...
      test_encoding = np.array(base_net.predict(test_image.reshape((1,28,28,1,1))))
    ...
    if typ == 'triplet': 
      ...
      for classe in range(10):
        ...
        encodings_of_known_data[classe,:] = np.array(base_net.predict(example.reshape((-1,28,28,1))))
      ...
      test_encoding = np.array(base_net.predict(test_image.reshape((-1,28,28,1))))
...
def transformed_encodings_plot_3d(pca,transformed_encodings,labels):
  ...
  fig.show()
  ################
def one_way_one_shotlearning (base_net,all_train,all_test , typ,size_of_encoding):
  ...
  if typ == 'contrastive' or typ =='binary_crossentropy': 
      ...
      test_class = np.random.randint(0,10) ## choose a test class
  ...
  if typ == 'triplet': 
      ...
      # print('shape_of_encodings',encodings_of_known_data.shape)
      test_class = np.random.randint(0,10) ## choose a test class
  ...
  subplot = fig.add_subplot(1,10+1,1)
  plotidx = 2
  plt.imshow(test_image.reshape(28,28),vmin=0, vmax=1,cmap='Greys')
  subplot.title.set_text("Test image")
  distances = []
  for j in range(10):
    distance = np.float(euclidean_distance((encodings_of_known_data[j],test_encoding ))[0][0])
    distances.append(distance)
    subplot = fig.add_subplot(1,10+1,plotidx)
    plt.imshow(known_data[j,:].reshape(28,28),vmin=0, vmax=1,cmap='Greys')
    subplot.title.set_text(("Class {0}\n{1:.3e}".format(j,distance)))
    plotidx += 1
...
def loss_comparison_plot(contrastive_history,binary_history):
  ...
  if len(contrastive_history.history['loss']) > len(binary_history.history['loss']) :
    ...
    binary_train_loss = binary_history.history['loss']
    binary_val_loss = binary_history.history['val_loss']
  plt.plot(x,contras_val_loss,'g',label='contrastive train')
  plt.plot(x,contras_train_loss,'g--',label='contrastive validation')
  plt.plot(x,binary_val_loss,'r',label='binary cross entropy train')
  plt.plot(x,binary_train_loss,'r--',label='binary cross entropy val')
